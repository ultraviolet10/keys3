...
export function PianoTilesGame() {
  ...
  const [gameStatus, setGameStatus] = useState<'idle' | 'playing' | 'paused' | 'gameOver'>('playing')
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: i'll study it all in a bit
    ...
    const gameLoop = (timestamp: number) => {
      ...
      setTileRows(prevRows => {
        ...
        const topMostTile = updatedRows.length > 0 
          ? updatedRows.reduce((top, row) => row.y < top.y ? row : top)
          : null
        
        if (!topMostTile || topMostTile.y >= 0) { // Spawn if no tiles exist or topmost tile is far enough down
  ...
  const extractCoordinates = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    // biome-ignore lint/style/noUselessElse: donno wont delete
    } else {
      // Desktop click
          ...
          {tileRows.flatMap(row => (
            ...
            [0, 1, 2, 3].map(columnIndex => {
              ...
              if (isActiveColumn) {
                ...
                switch (row.status) {
                  case 'pending': bgColor = 'bg-black'; break
                  case 'tapped': bgColor = 'bg-blue-500'; break
                  case 'missed': bgColor = 'bg-red-500'; break
                }
              }
